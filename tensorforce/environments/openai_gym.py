...
class OpenAIGym(Environment):
    ...
    def create_level(cls, level, max_episode_steps, reward_threshold, **kwargs):
            ...
            for name in env_specs:
                if search == name[: name.rindex("-v")]:
                    if level is None:
                        level = name
                    if (
                        max_episode_steps is False
                        and gym.envs.registry.env_specs[name].max_episode_steps
                        is not None
                    ):
                        continue
                    elif (
                        max_episode_steps
                        != gym.envs.registry.env_specs[name].max_episode_steps
                    ):
                        continue
                    level = name
                    break
                ...
                if level is None:
                    raise TensorforceError.value(
                        name="OpenAIGym", argument="level", value=level
                    )
        ...
        if max_episode_steps is None:
            ...
            if not (
                (max_episode_steps is False)
                and (gym.envs.registry.env_specs[level].max_episode_steps is None)
            ):
    ...
    def reset(self):
        ...
        if self.min_value is not None:
            states = np.clip(
                states, self.states_spec["min_value"], self.states_spec["max_value"]
            )
    ...
    def execute(self, actions):
        ...
        self.timestep += 1
        if (
            self._max_episode_timesteps is not None
            and self.timestep == self._max_episode_timesteps
        ):
            assert terminal
            terminal = 2
        elif terminal:
            assert (
                self._max_episode_timesteps is None
                or self.timestep < self._max_episode_timesteps
            )
    ...
    def specs_from_gym_space(
        ...
        if isinstance(space, gym.spaces.Discrete):
            ...
            if (space.nvec == space.nvec.item(0)).all():
                return dict(
                    type="int", shape=space.nvec.shape, num_values=space.nvec.item(0)
                )
            else:
                specs = dict()
    ...
    def unflatten_action(action):
        if not isinstance(action, dict):
            ...
            all(name.startswith("gymmdc") for name in action)
            or all(name.startswith("gymbox") for name in action)
            or all(name.startswith("gymtpl") for name in action)
        ):
            ...
            while True:
                if any(name.startswith(space_type + str(n) + "_") for name in action):
                    ...
                    actions.append(
                        OpenAIGym.unflatten_action(action=action[space_type + str(n)])
                    )
                else:
                    break
                n += 1
            if all(name.startswith("gymmdc") for name in action) or all(
                name.startswith("gymbox") for name in action
            ):
